...
import {
  send, getTransactions, getSingleTransaction, unconfirmedTransactions,
...
export const transactionsUpdateUnconfirmed = (
  ...
  return unconfirmedTransactions(liskAPIClient, address).then(response => dispatch({
    data: {
      failed: pendingTransactions.filter(
        tx => response.data.filter(unconfirmedTx => tx.id === unconfirmedTx.id).length === 0,
      ),
    },
...
export const loadTransactionsFinish = accountUpdated => (dispatch) => {
...
export const loadTransactions = ({ publicKey, address }) => (dispatch, getState) => {
  const liskAPIClient = getState().peers.liskAPIClient;
...
export const loadTransaction = ({ id }) => (dispatch, getState) => {
  const liskAPIClient = getState().peers.liskAPIClient;
