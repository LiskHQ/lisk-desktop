...
const searchDelegate = ({ publicKey, address }) => (dispatch, getState) => {
...
const searchVotes = ({ address }) => (dispatch, getState) => {
  const liskAPIClient = getState().peers.liskAPIClient;
  getAllVotes(liskAPIClient, address).then(response => dispatch({
...
export const searchMoreVoters = ({ address, offset = 0, limit = 100 }) => (dispatch, getState) => {
  const liskAPIClient = getState().peers.liskAPIClient;
...
export const searchAccount = ({ address }) => (dispatch, getState) => {
  const liskAPIClient = getState().peers.liskAPIClient;
...
export const searchSuggestions = ({ searchTerm }) => (dispatch, getState) => {
  const liskAPIClient = getState().peers.liskAPIClient;
  dispatch({
    data: {},
