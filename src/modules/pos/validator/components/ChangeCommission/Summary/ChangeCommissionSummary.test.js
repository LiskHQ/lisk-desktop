...
jest.mock('@transaction/manager/transactionSummary', () =>
  jest.fn(({ confirmButton, cancelButton }) => (
    <div>
      <button data-testid="confirm" onClick={confirmButton.onClick} >{confirmButton.label}</button>
        <button data-testid="cancel" onClick={cancelButton.onClick} >{cancelButton.label}</button>
    </div>
  ))
);
