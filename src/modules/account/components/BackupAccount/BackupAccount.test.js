...
describe('Backup account recovery phrase flow', () => {
  it('Should successfully go though the flow', async () => {
    renderWithRouter(BackupAccount, props);
    ...
    expect(screen.getByText('Keep it safe as it is the only way to access your wallet.')).toBeTruthy();
    expect(screen.getByText('Please carefully write down these 12 words and store them in a safe place.')).toBeTruthy();
    ...
    expect(screen.getByText('Please choose the correct words from the list below to complete your secret recovery phrase.')).toBeTruthy();
