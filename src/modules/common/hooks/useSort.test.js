...
describe('useSort hook', () => {
  ...
  it('sorts list by the default sort order', async () => {
    const { result } = renderHook(
      () => useSort({ defaultSort: 'prop2:desc', data }),
    );

    await waitFor(() => {
      expect(result.current.sort).toEqual('prop2:desc');
      expect(result.current.sortedData).toEqual(
        data.sort((a, b) => (b.prop2 > a.prop2 ? 1 : -1)),
      );
    });

    const { result: result2 } = renderHook(
      () => useSort({ defaultSort: 'prop1:desc', data }),
    );

    await waitFor(() => {
      expect(result2.current.sort).toEqual('prop1:desc');
      expect(result2.current.sortedData).toEqual(
        data.sort((a, b) => (b.prop1 > a.prop1 ? 1 : -1)),
      );
    });
  });
