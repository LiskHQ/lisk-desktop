...
export const EventType = ({ action }) => (
  <div className={`${grid['col-xs-1']} event-action ${styles.eventAction}`}>{action}</div>
);

export const EventModule = ({ module }) => (
  <div className={`${grid['col-xs-3']} event-action ${styles.eventAction}`}>{module}</div>
);

export const EventIndex = ({ id }) => <div className={grid['col-xs-1']}>{id}</div>;
