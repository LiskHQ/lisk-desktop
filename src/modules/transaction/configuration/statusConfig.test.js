...
describe('Transaction signature status', () => {
    ...
    it('should return transaction status for partially signed transaction', () => {
        ...
        const isSenderAccountMultisignature = false;
        const status = getTransactionStatus(account, transactions, isSenderAccountMultisignature);
        expect(status).toEqual('test');
    });

    it.skip('should return transaction status for fully signed transaction', () => {
        const account = {};
        const transactions = {
            txSignatureError: null,
            signedTransaction,
