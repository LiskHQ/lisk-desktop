...
const Blocks = ({
  ...
  const {
    data: blocks, error, isLoading, fetchNextPage, hasNextPage, refetch,
  } = useBlocks();
  const formatters = {
  ...
  const applyBlockFilters = (blockFilters) => {
    ...
    const usedFilters = Object.keys(filterObj).filter(key => filterObj[key] !== '').reduce((acc, key) => { acc[key] = filterObj[key]; return acc; }, {});
    refetch({ config: { params: transformParams(usedFilters) } });
  };
