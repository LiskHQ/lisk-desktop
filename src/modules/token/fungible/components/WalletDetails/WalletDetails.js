...
const WalletDetails = ({ t, wallet, token, className, isWalletRoute }) => {
  const tokens = Object.entries(wallet.info || {}).filter(([key, info]) => token.list[key] && info);
    ...
    <Box className={`${styles.box} ${className}`}>
      ...
      <BoxContent className={`${styles.container} coin-container`}>
        {tokens.map(([acctToken, info]) => (
          <BoxRow key={`${info.address}-${acctToken}`} className={`${styles.row} coin-row`}>
            <Link to={routes.wallet.path} className={styles.link}>
              <Icon name="lskIcon" />
              <div className={styles.details}>
                <span>{t('{{acctToken}} balance', { token: tokenMap[acctToken].label })}</span>
                <div className={styles.valuesRow}>
                  <DiscreetMode>
                    <div className={`${styles.cryptoValue} balance-value`}>
                      <div>
                        <TokenAmount val={info.summary?.balance} token={acctToken} />
                      </div>
                      <div>
                        <Converter
