...
describe('Dialog', () => {
  ...
  beforeEach(() => {
    history = {
      location: {
        pathname: `${routes.explorer}${routes.search.path}saved-accounts`,
        search: '',
      },
      push: sinon.spy(),
    };
  ...
  it('allows to close the dialog on regexp path ', () => {
    const basePath = `${routes.explorer}${routes.wallet.path}/1523498127498/`;
  ...
  it('allows to close the dialog on non-regexp path ', () => {
    history.location.pathname = `${routes.main}${routes.wallet.path}saved-accounts`;
    wrapper.setProps({ history });
    wrapper.find('.x-button').simulate('click');
