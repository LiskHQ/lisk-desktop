...
class AccountVisualWithAddress extends React.Component {
  ...
  render() {
    const {
      address, transactionSubject, moduleAssetId, size, truncate = 'small',
    } = this.props;
      ...
      <div className={`${styles.address}`}>
        {moduleAssetId !== MODULE_ASSETS_NAME_ID_MAP.transfer && transactionSubject === 'recipient' ? (
          ...
          <React.Fragment>
            ...
            <span className={`${styles.addressValue}`}>{truncate ? truncateAddress(transformedAddress, truncate) : transformedAddress}</span>
          </React.Fragment>
        )}
      </div>
    );
