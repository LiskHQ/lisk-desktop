...
export const visit = (value, history) => {
  if (value.match(regex.address)) {
    history.push(`${routes.explorer}${routes.account.path}/${value}`);
  } else if (value.match(regex.transactionId)) {
    history.push(`${routes.explorer}${routes.transaction.path}/${value}`);
  } else {
    history.push(`${routes.explorer}${routes.searchResult.path}/${value}`);
  }
};

export const visitAndSaveSearch = (value, history) => {
  if (value.length === 0) {
    history.push(`${routes.explorer}${routes.search.path}`);
  } else {
    value = value.trim();
    saveSearch(value);
    visit(value, history);
